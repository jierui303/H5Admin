...
class FinishedOrderController extends CommanController
{
     ...
     *获取待上门订单列表
     */
    public function getFinishedOrderLists()
    {
        try{
    ...
    private function getAllOrders($where)
    {
        ...
        if(!empty($this->getInfo)){
            ...
            foreach ($this->getInfo as $k=>$v){
                ...
                $newData[$k]['cleanRealName'] = $v['bnames'];//保洁员名称
                ...
                $newData[$k]['serviceTime'] = $v['servicestarttime'].'=至='.$v['serviceEndTime'];//保洁服务时间
